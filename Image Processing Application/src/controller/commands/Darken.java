...
/**
 * This is a class used for facilitating darkening operations on images, and
 ...
 */
public class Darken implements CommandController {
   ...
   * INVARIANT: the modification is > 0.
   * @param modification the desired modification to decrease image values.
   ...
   */
  public Darken(int modification, String name, String nfm) throws IllegalArgumentException {
    if (modification < 0) {
  ...
  /**
   * Executes the darkening operation on an image stored within the passed in model. First checks
   * if the file exists.
  ...
  public void execute(ImageProcessingModel model) throws NoSuchFileException {
    ...
    Color[][] result = model.makeDarker(image, modification);
